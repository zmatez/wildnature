package net.matez.wildnature.data.blockstates;

import net.matez.wildnature.data.setup.base.WNBlockstate;
import net.minecraft.resources.ResourceLocation;

public class WNBlockstate_Branch extends WNBlockstate {
    public WNBlockstate_Branch(String id) {
        super(id);
    }

    public WNBlockstate_Branch(ResourceLocation location) {
        super(location);
    }

    @Override
    public String getJSON() {
        return """
                {
                    "multipart": [
                        {   "when": { "up": "true" },
                            "apply": { "model": "%modid%:block/%name%_up" }
                        },
                        {   "when": { "down": "true" },
                            "apply": { "model": "%modid%:block/%name%_down" }
                        },
                        {   "when": { "north": "true" },
                            "apply": { "model": "%modid%:block/%name%_side", "uvlock": true }
                        },
                        {   "when": { "east": "true" },
                            "apply": { "model": "%modid%:block/%name%_side", "y": 90, "uvlock": true }
                        },
                        {   "when": { "south": "true" },
                            "apply": { "model": "%modid%:block/%name%_side", "y": 180, "uvlock": true }
                        },
                        {   "when": { "west": "true" },
                            "apply": { "model": "%modid%:block/%name%_side", "y": 270, "uvlock": true }
                        },
                        {   "when": { "waterlogged": "true"},
                            "apply": { "model": "%modid%:block/%name%_none"}
                        },
                        {   "when": { "waterlogged": "false"},
                            "apply": { "model": "%modid%:block/%name%_none"}
                        }
                    ]
                }
                """;
    }
}
