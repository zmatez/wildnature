    ...
    public static WNDevStructureSelection getOrCreate(ServerPlayer player, BlockPos clickPos, boolean use) {
        if (STRUCTURE_SELECTIONS.containsKey(player)) {
            ...
            if (selection.getLevel() == player.getLevel()) {
                if (use) {
                    WNChatUtil.send(player, "Set &vpos1&r to &v" + clickPos.getX() + " " + clickPos.getY() + " " + clickPos.getZ(), true);
                    selection.setPos1(clickPos);
                } else {
                    WNChatUtil.send(player, "Set &vpos2&r to &v" + clickPos.getX() + " " + clickPos.getY() + " " + clickPos.getZ(), true);
        ...
        if (use) {
            WNChatUtil.send(player, "Created and set &vpos1&r to &v" + clickPos.getX() + " " + clickPos.getY() + " " + clickPos.getZ(), true);
            selection.setPos1(clickPos);
        } else {
            WNChatUtil.send(player, "Created and set &vpos2&r to &v" + clickPos.getX() + " " + clickPos.getY() + " " + clickPos.getZ(), true);
            selection.setPos2(clickPos);
        }
