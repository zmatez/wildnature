...
import net.matez.wildnature.common.util.WNRenderUtils;
...
public class WNDevClientStructureSelection extends WNDevStructureSelection {
    ...
    public void render(Vec3 projectedView, float partialTicks, PoseStack poseStack) {
        ...
        if (isComplete()) {
            ...
            WNRenderUtils.drawBoundingBox(poseStack, buffer, relativeToPos1.x, relativeToPos1.y, relativeToPos1.z, relativeToPos2.x, relativeToPos2.y, relativeToPos2.z, 45, 210, 220, 255);
            ...
            for (BlockPos pos : centers) {
                ...
                WNRenderUtils.drawBoundingBox(poseStack, buffer, relativeToPos.x, relativeToPos.y, relativeToPos.z, relativeToPos.x + 1, relativeToPos.y + 1, relativeToPos.z + 1, centers.size() > 1 ? 165 : 20, centers.size() > 1 ? 35 : 165, centers.size() > 1 ? 25 : 85, 180);
                poseStack.popPose();
            }
            poseStack.popPose();
            ...
            poseStack.pushPose();
            WNRenderUtils.drawBoundingBox(poseStack, buffer, rPos1.x, rPos1.y, rPos1.z, rPos1.x + 1, rPos1.y + 1, rPos1.z + 1, 110, 35, 180, 120);
            poseStack.popPose();
            Vec3 rPos2 = new Vec3(getPos2().getX() - projectedView.x(), getPos2().getY() - projectedView.y(), getPos2().getZ() - projectedView.z());
            poseStack.pushPose();
            WNRenderUtils.drawBoundingBox(poseStack, buffer, rPos2.x, rPos2.y, rPos2.z, rPos2.x + 1, rPos2.y + 1, rPos2.z + 1, 210, 110, 35, 120);
            poseStack.popPose();
        } else {
            ...
            if (pos != null) {
                ...
                WNRenderUtils.drawBoundingBox(poseStack, buffer, relativeToPos.x, relativeToPos.y, relativeToPos.z, relativeToPos.x + 1, relativeToPos.y + 1, relativeToPos.z + 1, 220, 10, 60, 255);
                poseStack.popPose();
            }
        }
